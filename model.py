...
execution_times = [0.0004 , 0.0004 , 0.0004 ,  0.0004 , 0.0006 ,0.0006 , 0.0003 , 0.0003 , 0.0003, 0.0003, 0.0003, 0.0002, 0.0003 , 0.0004, 0.0005, 0.0005, 0.0004, 0.0004, 0.0005  , 0.0003, 0.0002, 0.0008, 0.0007, 0.0008 , 0.0003, 0.0004, 0.0004, 0.0004, 0.0003 , 0.0003, 0.0002, 0.0003 , 0.0004, 0.0004 , 0.0002 , 0.0003 , 0.0003 ,0.0005 , 0.0004  , 0.0003 ,   ]
...
new_queries = [
    ...
    # Add more new queries as needed
]
...
# Vectorize new queries
new_queries_vectorized = loaded_vectorizer.transform(new_queries)

# Predict execution times for new queries
predictions = loaded_model.predict(new_queries_vectorized)

# Print the predictions for each new query
for i, query in enumerate(new_queries):
    print(f"Query: {query}")
    print(f"Predicted Execution Time: {predictions[i]} seconds\n")

# Model evaluation using mean squared error on the training data
train_predictions = loaded_model.predict(X)
train_mse = mean_squared_error(execution_times, train_predictions)
